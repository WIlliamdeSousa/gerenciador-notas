...
class Disciplina(Opcao):
    ...
    def menu(self) -> None:
        ...
        separador()
        self.notas.mostrar()
        opcoes(None, self.opcoes_disc, 'O QUE DESEJA FAZER? ', limpar_tela=False)
...
def opcoes(
    ...
    mostrar_opcoes(texto, opcoes, limpar_tela)
    ler_opcao(mensagem_final, opcoes)
...
def ler_opcao(texto, opcoes) -> None:
    ...
    separador()
    opcoes[escolha].funcao()

def ordenar_disciplinas(disciplinas) -> list:
    ordenado = {}
    for disciplina in disciplinas:
        ordenado[disciplina.nome] = disciplina
    chaves_ordenadas = sorted(list(ordenado.keys()))
    disciplinas = [ordenado[i] for i in chaves_ordenadas]
    return disciplinas
